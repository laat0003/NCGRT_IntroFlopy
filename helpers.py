...
def ws7_model1(ws7,gis_f,model_f,plots_f):
    import os
    import sys
    import shutil
    import platform
    ...
    gwf = flopy.mf6.ModflowGwf(sim, modelname=model_name, save_flows=True, newtonoptions="under_relaxation")
    disv = flopy.mf6.ModflowGwfdisv(gwf,angrot=0,length_units="METERS", **gridprops_vg)
    grid_path = os.path.join(ws7,'Gridgen')
    if os.path.exists(grid_path): 
        shutil.rmtree(grid_path)
        os.mkdir(grid_path)
    else:
    ...
    maw_pdata[1] = pdata # all wells kick off at SS for this specific scenario which is stress period 4 in the model
